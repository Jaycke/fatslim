...
cdef class Topology(object):
    ...
    def get_atom_internal_id(self, fsl_int atomid):
        return self.fast_get_atom_internal_id(atomid)
...
cdef class Trajectory(object):
    ...
    def initialize(self, str hg_group="headgroups", str interacting_group="protein"):
        ...
        cdef fsl_int[:] hg_group_atomidwors, hg_bead_atomids_offsets, lipid_atomids_offsets, \
...
cdef class MakeIndex(object):

    cpdef CreateIndex(self,f_conf,f_output):
        f_conf = f_conf.encode()
        f_output = f_output.encode()
        close = False
        Temp_OutFile = ""
        ...
        try:
            ...
            print("Erreur avec le fichier " + f_output)

        print("Name all atoms you want in the index file \n"
              "Example : \n"
              "C1 \n"
              "C2 \n"
              "C3 \n"
        ...
        while close != True:
            valeur = raw_input()
            if valeur == "q" or valeur == "Q" : break
            Temp_OutFile = Temp_OutFile + "\n" + valeur

        for line in Temp_OutFile.rsplit("\n"):
            if len(line) != 0:
                str_line = "".join(line)
                i = 0
                with open(f_conf, "r") as fc:
                    while i < 3:
                        ref = fc.readline()
                        i += 1
                    if i < 3:
                        print ("blablabla")
                        open(f_output, "w").write("[ " + str_line + " ]\n")
                        First = True
                    else:
                        open(f_output, "a").write("\n[ " + str_line + " ]\n")
                        First = True
                    while ref != "":
                        ref = "".join(ref).rstrip(" ")
                        if ref.find("".join(str_line)) != -1:
                            ref = ("".join(ref).rstrip(" ").rsplit())
                            if len(ref[1].rstrip(" ")) == len(str_line):
                                if First:
                                    open(f_output, "a").write(ref[2])
                                    First = False
                                else:
                                    open(f_output, "a").write("\t" + ref[2])
                        ref = "".join(fc.readline()).rstrip(" ")
                fc.close()

cdef class OrderParameter(object):

    cpdef check_length(self, lenght, a, b):
        if lenght > 0.3:
            raise TypeError("Distance between atom ", a, " and ", b, " > 0.3nm")

    cpdef direction(self,direction):

        # Calculate axis :
        direction = self.namespace.direction
        if len(direction) > 0:
            if direction == "x":
                axis = 0
                print("Taking x axis as normal to the membrane")
            if direction == "y":
                axis = 1
                print("Taking y axis as normal to the membrane")
            if direction == "z":
                axis = 2
                print("Taking z axis as normal to the membrane")
        else:
            ValueError
        return axis

    cpdef Check_Atom(self, index):
        ...
        for i in range (len(ligne)-1):
            if len(ligne[i].split()) != len(ligne[i+1].split()) and (i+1 < len(ligne)-1):
                raise TypeError("Les groupes n'ont pas tous les mêmes nombre d'atomes")
            ligne[i]=ligne[i].split()
        return ligne

    cpdef Calc_OrderParam(self,coords_atome,index,topology):

        frame = 1
        groups = self.Check_Atom(index)
        frameorder = [0, 0, 0]
        Cossum = [0, 0, 0]
        order = []
        param_order = []
        size = int(groups[0][1]) - int(groups[0][0])
        order.append([0] * 3)
        param_order.append([0])
        for i in range (1,len(groups)-2):
            ...
            for j in range(len(groups[i])):
                ...
                a1=int(groups[i][j])
                a2=int(groups[i+1][j])
                #Previous internal id atome
                P_Internal_Atome = topology.get_atom_internal_id(topology.get_atom(a0)[3])
                #Actual internal id atome
                A_Internal_Atome = topology.get_atom_internal_id(topology.get_atom(a1)[3])
                #Next internal id atome
                N_Internal_Atome = topology.get_atom_internal_id(topology.get_atom(a2)[3])
                ...
                if j >= 0:
                    while P_Internal_Atome > 46:
                        P_Internal_Atome = P_Internal_Atome - 46
                        x+=1
                    while A_Internal_Atome > 46:
                        A_Internal_Atome = A_Internal_Atome - 46
                        y+=1
                    while N_Internal_Atome > 46:
                        N_Internal_Atome = N_Internal_Atome - 46
                        z+=1

                    PreviousCoord = coords_atome[x][P_Internal_Atome]
                    ActualCoord = coords_atome[y][A_Internal_Atome]
                    NextCoord = coords_atome[z][N_Internal_Atome]

                    dist = (NextCoord[0] - PreviousCoord[0],
                            NextCoord[1] - PreviousCoord[1],
                            NextCoord[2] - PreviousCoord[2])

                    Length = sqrt((dist[0]) ** 2 + (dist[1]) ** 2 + (dist[2]) ** 2)
                    self.check_length(Length,a0,a2)
                    ...
                    # Sx
                    temp1 = (NextCoord[0] - ActualCoord[0],
                             NextCoord[1] - ActualCoord[1],
                             NextCoord[2] - ActualCoord[2])

                    temp2 = (PreviousCoord[0] - ActualCoord[0],
                             PreviousCoord[1] - ActualCoord[1],
                             PreviousCoord[2] - ActualCoord[2])

                    Sx = (temp1[1] * temp2[2] - temp1[2] * temp2[1],
                          temp1[2] * temp2[0] - temp1[0] * temp2[2],
                          temp1[0] * temp2[1] - temp1[1] * temp2[0])

                    Norm = sqrt((Sx[0]) ** 2 + (Sx[1]) ** 2 + (Sx[2]) ** 2)
                    Sx = (Sx[0] * (1 / Norm),
                          Sx[1] * (1 / Norm),
                          Sx[2] * (1 / Norm))

                    # Sy
                    Sy = (float(Sz[1]) * Sx[2] - float(Sz[2]) * Sx[1],
                          float(Sz[2]) * Sx[0] - float(Sz[0]) * Sx[2],
                          float(Sz[0]) * Sx[1] - float(Sz[1]) * Sx[0])

                    Norm = sqrt((Sy[0]) ** 2 + (Sy[1]) ** 2 + (Sy[2]) ** 2)
                    ...
                    for m in range(3):
                        frameorder[m] += round(0.5 * (3 * Cossum[m] - 1), 6)

            for m in range(3):
                order[i][m] += round((frameorder[m] / size), 6)
            del frameorder
            frame += 1

        for i in range(1,len(groups)-2):
            order[i][0] = round(order[i][0]*(1.0 / frame+1),6)
            order[i][1] = round(order[i][1]*(1.0 / frame+1),6)
            order[i][2] = round(order[i][2]*(1.0 / frame+1),6)

        for i in range(1,len(groups)-2):
            param_order[i] = round(-1 * (0.6667 * order[i][0] + 0.333 * order[i][1]), 6)

        return param_order

    cpdef Plot_Order(self,output,param_order):

        try:
            open(output, "w").write("")
        except IOError:
            print("Erreur avec le fichier " + output)

        with open(output, "a") as op:
            print("Deuterium order parameter save to file %s" %(output))
            for i in range(1, len(param_order)):
                op.write(str(i) + "\t" + str(param_order[i]) + "\n")
        op.close()